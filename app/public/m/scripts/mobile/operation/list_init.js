...
function inflate_listTop(){
    ...
    for (let i = 0; i < data.teamArgArray.length; i++) {
        ...
        if(lastRendering.bus===i+1){
            bustxt+='<div class="ol_bus_team ol_bus_box ol_bus_box--selected" tid="'+data.teamArgArray[i]+'"><div class="ol_bus_team_top"><p class="ol_bus_team_busno">BUS '+(i+1)+'</p>'
        }else{
            bustxt+='<div class="ol_bus_team ol_bus_box" tid="'+data.teamArgArray[i]+'"><div class="ol_bus_team_top"><p class="ol_bus_team_busno">BUS '+(i+1)+'</p>'
        }
        ...
        if(data.teams[data.teamArgArray[i]].guide){
            ...
            for (let j = 0; j < data.teams[data.teamArgArray[i]].guide.length; j++) {
                if(guideData[data.teams[data.teamArgArray[i]].guide[j]]){
                    bustxt+=guideData[data.teams[data.teamArgArray[i]].guide[j]].name + ", "
                }else{
                    bustxt+="deleted guide, "
                }
            }
