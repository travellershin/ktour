...
function inflate_reservation(){
    ...
    for (let i = 0; i < rData.length; i++) {
        ...
        if(draw){
            ...
            txt += '<div class="rv_content" id="'+rData[i].id+'"><div class="rv_liner">'
            if(rData[i].star){
                txt+='<div class="rv_content_star rv_content_star--on"></div>'
            }else{
                txt+='<div class="rv_content_star"></div>'
            }
            txt+='<p class="rv_content_product">'+rData[i].product.split("_")[2] + '</p><p class="rv_content_date">'+rData[i].date + '</p></div>'

            txt+='<div class="rv_liner"><p class="rv_content_agency">'+rData[i].agency + '</p><p class="rv_content_name">'+rData[i].clientName + '</p>'


            txt += '<p class="rv_content_people">' +rData[i].people+' ('+rData[i].adult+'/'+rData[i].kid+')' +'</p></div>'

            if(rData[i].memo){
                if(rData[i].memo==="N/A"){rData[i].memo=""}
            }
            txt += '<p class="rv_content_memo" title="'+rData[i].memo+'">'+rData[i].memo+'</p></div>'
        }else{
            //console.log(rData[i])
        }
    }
