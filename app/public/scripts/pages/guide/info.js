...
function gInfo_detail(gid){
    ...
    if(data.language){
        for (let i = 0; i < data.language.length; i++) {
            $("#gel_"+data.language[i]).addClass("g_e_lang--checked")
        }
    }
    if(data.preferDay){
        for (let i = 0; i < data.preferDay.length; i++) {
            $("#gep_"+data.preferDay[i]).addClass("g_e_prefer--checked")
        }
    }
    if(data.driver){
        $(".g_e_input_driver").val("O")
    }else{
